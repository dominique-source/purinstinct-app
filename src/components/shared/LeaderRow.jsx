...
export function LeaderRow({player,rank,highlight,isMe,onOpen}){
  ...
  return(
    <div onClick={onOpen||undefined} style={{display:"flex",alignItems:"center",gap:12,padding:"8px 12px",borderRadius:12,
      background:highlight?"#1a2e0555":"transparent",border:highlight?"1px solid #B8E02025":"1px solid transparent",
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
          ...
          {isMe&&<span style={{color:"#B8E020",fontSize:11}}>(toi)</span>}
          {elig&&<span style={{background:"#B8E02020",color:"#B8E020",fontSize:10,padding:"1px 5px",borderRadius:4}}>✓</span>}
        </div>
        <div style={{display:"flex",gap:3}}>{ZK.map(zk=><ZonePip key={zk} zone={zk} played={(player.zonesPlayed||[]).includes(zk)}/>)}</div>
      </div>
      <div style={{textAlign:"right",flexShrink:0}}>
        <div className={flash?"flash-change":""} style={{...S.display(22),color:highlight?"#B8E020":"#e5e7eb"}}>{pts}</div>
